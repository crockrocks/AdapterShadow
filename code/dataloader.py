...
class DataFiles():
    def __init__(self, args):
        ...
        if args.dataset_name == 'sbu':
            ...
            train_dir = root_dataset.joinpath('SBUTrain4KRecoveredSmall', 'images')
            train_imgs = train_dir.rglob('*.jpg')
            self.train_jpgs = [(p, root_dataset.joinpath('SBUTrain4KRecoveredSmall', 'labels', p.name[:-3]+'png')) for p in train_imgs]
            
            test_dir = root_dataset.joinpath('SBU-Test_rename', 'ShadowImages')
            test_imgs = test_dir.rglob('*.jpg')
            self.test_jpgs = [(p, root_dataset.joinpath('SBU-Test_rename', 'ShadowMasks', p.name[:-3]+'png')) for p in test_imgs]
